...
def edit(list_of_concerns, video_name, quality_factor, file_mode='RAM', check_intervals=10):
...
def edit_photo(list_of_concerns, photo_name, quality_factor):
    ...
    res = int(original_res*quality_factor)
    path = "store_photos_after_split"
    result = detect.detect_and_edit(list_of_concerns, original_res, [
                                    image], ratio, path, res, language='ell', mode='RAM')
    image = result[0]
