...
class Censor():
    def __init__(self, api_controller_path, url, storage_mode='RAM'):
        self.url = url
    ...
    def censor(self, file_path, keywords, options):
        ...
        os.remove(self.api_controller_path.replace(
            'api_controller.py', '')+'video_under_procces.mp4')
    ...
    def censor_photos(self, user_path, keywords, options):
        if os.path.isdir(user_path):
            ...
            path_to_clear = self.api_controller_path.replace(
                'api_controller.py', '')+'store_output_photo/'
            shutil.rmtree(path_to_clear)
            os.remove(self.api_controller_path.replace(
                'api_controller.py', '')+'photo_under_procces.png')
            return results
        else:
...
def initialize(location, url, storage_mode):
    My_censor = Censor(location, url, storage_mode)
    return My_censor
